...
document.addEventListener("DOMContentLoaded", () => {
    ...
    (document.querySelector("#left_button") as HTMLElement).addEventListener("click", evt => {
        ...
        for (let i = 0; i < imageElements.length; i++) {
            if (i < image_elements.length) {
                (imageElements[i] as HTMLImageElement).src = image_elements[i];
            }
        }
    });

    // click event listener for the Right button    
    (document.querySelector("#right_button") as HTMLElement).addEventListener("click", evt => {
